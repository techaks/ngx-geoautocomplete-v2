...
export class AutoCompleteSearchService {
  ...
  getPredictions(url: string, query: string): Promise<any> {
    return new Promise(resolve => {
      this._http
        .get(url, { params: new HttpParams().set("query", query) })
        .subscribe((data: any) => {
          if (data) {
            resolve(data);
          } else {
  ...
  getLatLngDetail(url: string, lat: string, lng: string): Promise<any> {
    return new Promise(resolve => {
      this._http
        .get(url, { params: new HttpParams().set("lat", lat).set("lng", lng) })
        .subscribe((data: any) => {
          if (data) {
            resolve(data);
          } else {
  ...
  getPlaceDetails(url: string, placeId: string): Promise<any> {
    return new Promise(resolve => {
      this._http
        .get(url, { params: new HttpParams().set("query", placeId) })
        .subscribe((data: any) => {
          if (data) {
            resolve(data);
          } else {
  ...
  private getUniqueResults(arr: any): any {
    return Array.from(
      arr.reduce((m, t) => m.set(t.place_id, t), new Map()).values()
    );
  }
